            ...
            <form method="POST" action="{{ route('jobs.update', $job) }}" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="notes" rows="3"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">{{ $job->notes }}</textarea>
                </div>

                <button type="submit"
                    class="w-full bg-blue-600 text-white py-2 rounded-lg text-sm font-medium hover:bg-blue-700">Update
                    Application</button>
            </form>
        </div>
    </div>
