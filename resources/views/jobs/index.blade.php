...
<body class="bg-gray-100 min-h-screen">
    <nav class="bg-white shadow px-6 py-4 flex justify-between items-center">
        ...
        <a href="{{ route('jobs.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700">+ Add Job</a>
    </nav>
            ...
            <div class="bg-white rounded-xl shadow p-5 mb-4 flex justify-between items-center">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <form method="POST" action="{{ route('jobs.destroy', $job) }}">
                        @csrf
                        @method('DELETE')
                        <button type="submit" onclick="return confirm('Delete this?')" class="text-sm text-red-400 hover:underline">Delete</button>
                    </form>
                </div>
            </div>
        @empty
            <div class="bg-white rounded-xl shadow p-10 text-center text-gray-400">
                No applications yet. <a href="{{ route('jobs.create') }}" class="text-blue-500 hover:underline">Add your first one!</a>
            </div>
        @endforelse
    </div>
</body>
